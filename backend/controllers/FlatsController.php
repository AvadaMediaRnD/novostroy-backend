...
use backend\controllers\ZController as Controller,
    backend\models\FlatSearch,
    backend\models\FlatImportForm,
    backend\models\FlatForm;
...
class FlatsController extends Controller {
    ...
    public function actionUpdate($id, $payment_id = null) {
        ...
        if(Payment::find()->joinWith('invoices')->where(['payment.flat_id' => $model->id, 'invoice.status' => Invoice::STATUS_COMPLETE])->count() <= 1) {
            $isPaid = false;
        }
        else {
            $isPaid = true;
        ...
        if(Payment::find()->where(['payment.flat_id' => $model->id])->one() === null) {
            $isPlan = false;
        }
        else {
            $isPlan = true;
    ...
    public function actionUpdatePaymentsMany($id) {
        ...
        if ($model && Yii::$app->request->post()) {
            ...
            /*if (Payment::find()->joinWith('invoices')->where(['payment.flat_id' => $model->id, 'invoice.status' => Invoice::STATUS_COMPLETE])->one() !== null) {
                Yii::$app->session->setFlash('error', 'Данные не могут быть сохранены так, как есть проведённые платежи!');
                return $this->redirect(['update', 'id' => $id]);
            } else {
                $arrPaments = \yii\helpers\ArrayHelper::getColumn(Payment::find()->where(['payment.flat_id' => $model->id])->asArray()->all(), 'id');
                Invoice::deleteAll(['in', 'payment_id', $arrPaments]);
                Payment::deleteAll(['payment.flat_id' => $model->id]);
            }
             * 
             */
    ...
    public function actionAjaxGetPaymentManyPrice($flat_id, $count = 0) {
        ...
        $priceFull = round($flat->getPrice(), 2);
        $priceSel = round($flat->getPriceSell(), 2);

        if ($priceSel > 0) {
            $saldoSum = abs($priceSel) - abs($flat->getPayments()->sum('price_fact'));
        } else {
            $saldoSum = abs($priceFull) - abs($flat->getPayments()->sum('price_fact'));
        }

        $paymentPriceLeft = round($saldoSum / $count, 2);

        Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;

        return [
            'price_plan' => $paymentPriceLeft,
        ];
    }
    ...
    public function actionAjaxGetItems($flat_id = null) {
        ...
        if ($flatModel) {
            ...
            $paymentsQuery = Payment::find()->where(['flat_id' => $flatModel->id])->andWhere(['<','pay_date',$dateFilter]);
            foreach ($paymentsQuery->all() as $payment) {
                $paymentsData .= '<option value="' . $payment->id . '">№ ' . $payment->pay_number . ' от ' . $payment->pay_date . '</option>' . "\r\n";
            }
